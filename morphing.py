# -*- coding: utf-8 -*-
"""
Created on Sat Jan 23 19:05:00 2021

@author: Dell

https://github.com/cirbuk/face-morphing
https://github.com/andrewdcampbell/face-movie
https://livecodestream.dev/post/detecting-face-features-with-python/
https://www.pyimagesearch.com/2017/05/22/face-alignment-with-opencv-and-python/
https://inst.eecs.berkeley.edu/~cs194-26/fa17/upload/files/proj4/cs194-26-aak/
https://alliance.seas.upenn.edu/~cis581/Lectures/Fall2019/cis581-Morph.pdf
"""
...
class Landmarks:
    ...
    def align_images(self,index, index1, im1, im2, border, prev=None):
        ...
        #cv2.imwrite(os.path.join(r'F:\ML\Image_Color\Morphing\Tank' +r'\_'+str(index)+'.jpg'),warped_im2)
...
class Morph:
    def __init__(self,s0,s1,images,img_pt,tri_pt,loop_back):
        self.s0=s0
        self.s1=s1
        self.images=images
        self.img_pt=img_pt
    ...
    def generate_morph(self,duration,frame_rate,transition_rate,video_path):
        ...
        for i in range(len(self.images)-1):
            last = self.morphing(i,i+1,transition_rate, frame_rate, video_path, p)
        ...
        if not self.loop_back:
            """
            stream = ffmpeg.input(r'F:\ML\Image_Color\Morphing\dusk.mp3').filter_('atrim', start = 0, end = int(int(total_frames/frame_rate)/3))#.filter_('asetpts', 'PTS-STARTPTS')
            stream1 = ffmpeg.input(r'F:\ML\Image_Color\Morphing\skin.mp3').filter_('atrim', start = int(int(total_frames/frame_rate)/3), end = 2*int(int(total_frames/frame_rate)/3))#.filter_('asetpts', 'PTS-STARTPTS')
            vstream = ffmpeg.input(r'F:\ML\Image_Color\Morphing\avni2.mp4')
            ffmpeg.concat(vstream, stream, v=1, a=1).output(r'F:\ML\Image_Color\Morphing\avni3.mp4').run()
            
            stream1 = ffmpeg.input(r'F:\ML\Image_Color\Morphing\skin.mp3').filter_('atrim', start = int(int(total_frames/frame_rate)/3), end = 2*int(int(total_frames/frame_rate)/3))#.filter_('asetpts', 'PTS-STARTPTS')
            vstream1 = ffmpeg.input(r'F:\ML\Image_Color\Morphing\avni3.mp4')
            ffmpeg.concat(vstream1, stream1, v=1, a=1).output(r'F:\ML\Image_Color\Morphing\avni4.mp4').run()
            """
            stream2 = ffmpeg.input(r'/Users/nimishamittal/Documents/OldProjects/ML/Image_Color/Morphing/better_dance.mp3').filter_('atrim', start = 0, end = int(total_frames/frame_rate)).filter_('asetpts', 'PTS-STARTPTS')
            vstream2 = ffmpeg.input(r'/Users/nimishamittal/Documents/OldProjects/ML/Image_Color/Morphing/emma.mp4')
            ffmpeg.concat(vstream2, stream2, v=1, a=1).output(r'/Users/nimishamittal/Documents/OldProjects/ML/Image_Color/Morphing/emma_main.mp4').run()
        else:
            print("in",video_path)
            ff = ffmpy.FFmpeg(
            	inputs = {video_path : None},
            	outputs = {"loop_painting.gif" : None})
            
            ff.run()

            
    def morphing(self,ind1,ind2,duration,frame_rate,video_path,p):
        total = int(duration*frame_rate)
        global total_frames
        total_frames+=total
        print("total",total)
        for i in range(total):
            ...
            if self.loop_back:
                ...
                if self.alpha > 1 :  self.alpha = 2 - self.alpha
            ...
            for e in range(len(self.tri_pt[ind1])):    
                x = int(self.tri_pt[ind1][e][0])
                y = int(self.tri_pt[ind1][e][1])
                z = int(self.tri_pt[ind1][e][2])
            ...
            if not self.loop_back:
                cv2.imwrite(os.path.join(r'/Users/nimishamittal/Documents/OldProjects/ML/Image_Color/Morphing/result/frames4' +r'/morph'+str(i)+'.jpg'),cv2.cvtColor(temp_res,cv2.COLOR_RGB2BGR))
    ...
    def triangle_morphing(self,im1,im2,area,t1,t2,t):
        ...
        # Applying transformation to the output
...
def load_images_from_folder(folder):
    ...
    return images
#read_img = [cv2.imread(r'F:\ML\Image_Color\Morphing\lady_ermine.jpg',cv2.IMREAD_COLOR),cv2.imread(r'F:\ML\Image_Color\Morphing\british_venus.jpg',cv2.IMREAD_COLOR)]#,cv2.imread(r'F:\ML\Image_Color\Morphing\sam4.jpg',cv2.IMREAD_COLOR)]

#images = [r'F:\ML\Image_Color\Morphing\face-movie-master\face-movie-master\demos\daniel\2000_01.jpg',r'F:\ML\Image_Color\Morphing\face-movie-master\face-movie-master\demos\daniel\2004_01.jpg',r'F:\ML\Image_Color\Morphing\face-movie-master\face-movie-master\demos\daniel\2002.jpg']
read_img=[]
#for img in images:
#    read_img.append(cv2.imread(str(img),cv2.IMREAD_COLOR))
read_img = load_images_from_folder(r'/Users/nimishamittal/Documents/OldProjects/ML/Image_Color/Morphing/Emma')
print(read_img)
    #r'F:\ML\Image_Color\Morphing\face-movie-master\face-movie-master\demos\daniel')
 
                                   
                                  
prev=None
#assigning looping effect
loop = False

#assigning overlay effect
overlay = True
...
for img in read_img:
    if overlay:
        prev=c.align_images(index,t_index,target,img, 5,prev)
    else:
...
morph = Morph(aligned[0].shape[0],aligned[0].shape[1],aligned,pts,del_pt,loop)
morph.generate_morph(1.4,25,4,'emma.mp4')
#print("s",s)